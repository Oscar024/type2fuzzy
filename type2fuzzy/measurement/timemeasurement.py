...
class TimeMeasurement(object):
    ...
    def __init__(self, report_folder, iterations=10):
        ...
        iterations          -- int, number of times function is repeated in order to calculate
                                average
        '''
    ...
    def __call__(self, func):
        ...
        def wrapped_func(*args):
            ...
            item_logger = self.setup_logger('item_logger', ('{}\\{}.{}.log'.format(dirname,filename, 
                datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d-%H%M%S"))))

            # loop iteration times
            for i in range(self.iterations):
                #start timer, execure function, stop timer
                ...
                item_logger.debug(('\t{},\t{}\t{},\t{}'.format(i, filename, returns, duration )).replace(']', '').replace('[', ''))
                print('{}...OK'.format(filename))

            #create a logger to add average duration and log
            summary_logger = self.setup_logger('summary_logger', ('{}\\time_measurement_report.log'.format(dirname)))
            summary_logger.debug(('\t{},\t{}'.format(filename, sum(durations)/len(durations))).replace(']', '').replace('[', ''))
